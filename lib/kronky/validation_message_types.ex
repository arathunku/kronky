defmodule Kronky.ValidationMessageTypes do
  ...
  object :validation_option do
    field :key, non_null(:string), description: @descs.option_key do
      resolve(&key_resolver/3)
    end

    field(:value, non_null(:string), description: @descs.option_value) do
      resolve(&value_resolver/3)
    end
  end
  ...
  def key_resolver({key, _value}, _args, %{adapter: adapter}) do
    {:ok, adapter.to_external_name(Atom.to_string(key), :field)}
  end

  def value_resolver({_key, value}, _args, %{adapter: adapter}) do
    {:ok, value}
  end
end
